  ...
  <div>
    ...
    >
      <template #custom-tool>
        <a-button type="primary" :disabled="selectedKeys.length === 0" @click="handleReupload"
          >重新上传</a-button
        >
      </template>
    </ManagePage>
    ...
    >
      <a-form layout="vertical">
        <a-form-item label="当前选中文件">
          <div>
            {{ selectedRows[0]?.fileName || '-' }} （key: {{ selectedRows[0]?.fileKey || '-' }}）
          </div>
        </a-form-item>
...
import { fileResourceStore, searchOptions } from '../../stores/fileResource-store'
...
const columns: TableColumnType<any>[] = [
  ...
  { title: '上传者', dataIndex: 'creatorId', width: '100px' },
  {
    ...
    dataIndex: 'isDeleted',
    width: '100px',
    customRender: ({ text }) => formatDeleted(text),
  },
  {
    title: '创建时间',
...
const handleEditOk = async () => {
  ...
  try {
    ...
    if (target.bizId !== undefined && target.bizId !== null)
      form.append('bizId', String(target.bizId))
