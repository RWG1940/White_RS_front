  ...
  <ScrollContent>
    <template #content>
      ...
      >
        <a-alert
          message="使用说明"
          description="上传表格文件、批次号，选择要关联的客户，系统将导入数据（若批次号已存在则更新数据）。"
          type="info"
          show-icon
      ...
      >
        <template #default>
          <div>
            <a-alert
              message="使用说明"
              description="用于管理批次号及对应的数据，可以理解为批量删除该批次的数据。"
              type="info"
              show-icon
              style="margin-bottom: 16px"
            />
            <a-tag
              v-for="batch in tableImportStore.list as any"
              :key="batch.id"
              color="blue"
              :closable="true"
