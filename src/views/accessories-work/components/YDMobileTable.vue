        ...
        <div class="search-bar">
            ...
            <div class="search-section">
                <a-row :gutter="[8, 8]">
                    ...
                    <a-col :span="5">
                        <a-button type="primary" @click="handleSearchReset">
                            <ReloadOutlined />
                        </a-button>
                    </a-col>
                </a-row>
                ...
                <a-row :gutter="[8, 8]">
                    ...
                    <a-col :span="6">
                        <div class="edit-button">
                            ...
                            <a-button type="primary" danger style="width: 100%;" @click.stop="deleteItem(item.id as number)" ><DeleteOutlined />
                            </a-button>
        ...
        <a-modal title="编辑记录" :open="editModalOpen" @ok="handleEditSave" @cancel="handleEditCancel" ok-text="确认"
            ...
            <a-form layout="vertical" class="edit-modal-form">
                <a-form-item v-for="field in editableFields" :key="field" :label="getFieldLabel(field)">
                    <!-- 洗标优先级 -->
                    <a-select v-if="field === 'washPriority'" v-model:value="editForm[field]" placeholder="选择洗标优先级">
                    ...
                    <!-- 吊牌优先级 -->
                    <a-select v-else-if="field === 'tagPriority'" v-model:value="editForm[field]" placeholder="选择吊牌优先级">
                        <a-select-option :value="0">正常做</a-select-option>
                        <a-select-option :value="1">有点着急</a-select-option>
                        <a-select-option :value="2">非常着急安排优先</a-select-option>
                    </a-select>
                    ...
                    <!-- 普通输入 -->
                    <a-input v-else-if="!field.includes('Time')" v-model:value="editForm[field]"
...
import { ref, computed, watch, reactive,createVNode } from 'vue'
...
import { EyeOutlined, ReloadOutlined, DeleteOutlined, ExclamationCircleOutlined  } from '@ant-design/icons-vue'
...
// 可编辑字段（参考 YDTable 的 editableFields）
...
// 过滤数据源 - 与YDTable的filteredDataSource逻辑一致
