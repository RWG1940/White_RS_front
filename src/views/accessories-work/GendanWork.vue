<template>
    <div>
        ...
        <!-- pc 端 -->
        <div v-else>
            <GDTable v-model:openHistory="openHistory" />
        </div>
        ...
        <!-- 订单管理弹窗：季度管理 -->
        <Modal :visible="openHistory" @update:visible="openHistory = $event" title="季度管理">
...
import { accStore, fetchPageByImportId, getQuarters, quarters,selectedBatchId } from '@/stores/acc-store';
...
const handleToggleQuarterStatus = async (quarterStr: string, status: number) => {
    ...
    fetchPageByImportId(selectedBatchId.value || 0, accStore.currentPage, accStore.pageSize, '')
}
