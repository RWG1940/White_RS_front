    ...
    >
      <template #cell-status="{ record, isEditing, editableData, getInternalKey }">
        <template v-if="!isEditing">
          <a-tag :color="record.status == 0 ? 'grey' : record.status == 1 ? 'green' : 'red'">
            {{ record.status == 0 ? '未付款' : record.status == 1 ? '支付成功' : '支付失败' }}
          </a-tag>
        </template>
        <template v-else>
...
import { userBillStore, columns, editableFields } from '@/stores/userBillStore'
import { generateName } from '@/utils/randomStr'
import { ref, onMounted, reactive, computed } from 'vue'
import { message } from 'ant-design-vue'

const store = userBillStore
