    ...
    <div class="mobile-form-container">
      <div class="mobile-form-card">
        ...
        <div class="form-group file-upload">
          ...
          <label for="fileInput" class="file-upload-label"> 点击选择图片文件 </label>
          <p style="color: red; background-color: yellow">
            需要清晰的人脸实拍照片，确保人脸在照片中心，请勿上传合照
          </p>
          <div v-if="imagePreview">
            <img :src="imagePreview" class="preview" />
            <div class="file-info">已选择: {{ file?.name || '' }}</div>
          </div>
        </div>
...
const addPerson = async () => {
  ...
  try {
    ...
    const res = await axios.post('/brm/person/add', payload, {
    ...
    if (res.data.success == true) {
      ...
      alert(`添加失败: ${res.data.errMsg || '未知错误'}`)
    }
