<template>
  <div class="prompts-container">
    ...
    <div class="prompts-cards-section">
      <!-- 复用提示词卡片--人像摄影 -->
...
const compositionCardProduct = computed(() =>
  generateCardData('构图与视角(产品)', '构图与视角配置'),
)
...
const postProcessingCardProduct = computed(() =>
  generateCardData('后期与商业风格', '后期与商业风格配置'),
)
// 风景摄影提示词卡片数据
const landscapeCard = computed(() => generateCardData('场景主体配置', '场景主体配置'))
const placeCard = computed(() => generateCardData('地理环境配置', '地理环境配置'))
const compositionCardLandscape = computed(() =>
  generateCardData('构图与视角配置', '构图与视角配置'),
)
const lightingCardLandscape = computed(() => generateCardData('光线与时间配置', '光线与时间配置'))
const weatherCard = computed(() => generateCardData('天气与自然元素配置', '天气与自然元素配置'))
const colorStyleCardLandscape = computed(() =>
  generateCardData('色彩与风格配置', '色彩与风格配置'),
)
const seasonCard = computed(() => generateCardData('季节与氛围配置', '季节与氛围配置'))
const postProcessingCardLandscape = computed(() =>
  generateCardData('后期与艺术风格配置', '后期与艺术风格配置'),
)
...
const handleInput = () => {
  ...
  // 检查是否需要自动配置提示词
  if (promptsStore.autoConfigPrompts && inputValue.value.trim().length > 0) {
    // 用户开始输入需求时，自动配置
    promptsStore.autoConfigurePrompts()
    console.log('自动配置提示词已触发，输入内容:', inputValue.value)
  }
}
...
const handleCardClear = () => {
  // 这个事件现在由各个卡片组件自行处理
  // 每个卡片只会清空自己的提示词，不会影响其他卡片
  console.log('卡片已清空其内部提示词')
}

// 重新绘制所有连接线的函数
const redrawAllLines = () => {
  // 触发所有提示词卡片的连接线重绘
  const event = new CustomEvent('redrawLines')
...
onMounted(() => {
  ...
  // 监听主容器的滚动事件，重新绘制连接线
  const contentContainer = document.querySelector('.prompts-container')
  if (contentContainer) {
    contentContainer.addEventListener('scroll', redrawAllLines)
  }

  // 也监听全局滚动事件
  window.addEventListener('scroll', redrawAllLines)
})
...
onUnmounted(() => {
  ...
  const contentContainer = document.querySelector('.prompts-container')
  if (contentContainer) {
    contentContainer.removeEventListener('scroll', redrawAllLines)
  }
  window.removeEventListener('scroll', redrawAllLines)
})
</script>
...
.custom-textarea:focus {
  outline: none;
  border-color: #18d5ff;
  box-shadow: 1px 1px 10px rgba(68, 239, 255, 0.474);
...
.custom-textarea {
  ...
  scrollbar-color: rgba(24, 213, 255, 0.5) transparent;
}
...
.custom-textarea::-webkit-scrollbar-thumb {
  background: linear-gradient(180deg, rgba(24, 213, 255, 0.8), rgba(24, 213, 255, 0.4));
  border-radius: 6px;
  border: 2px solid transparent; /* 拉开视觉距离 */
  background-clip: padding-box;
  transition: all 0.25s ease;
}

.custom-textarea::-webkit-scrollbar-thumb:hover {
  background: linear-gradient(180deg, rgba(24, 213, 255, 1), rgba(24, 213, 255, 0.7));
}

.custom-textarea::-webkit-scrollbar-thumb:active {
  background: rgba(24, 213, 255, 1);
}
...
.photo-type-btn.active {
  ...
  border-color: #18d5ff;
  color: #fff;
  box-shadow: 0 2px 8px rgba(24, 213, 255, 0.684);
}
...
.bulb-glow {
  ...
  filter: drop-shadow(0 0 4px rgba(24, 213, 255, 0.8)) drop-shadow(0 0 8px rgba(24, 213, 255, 0.5));
}
