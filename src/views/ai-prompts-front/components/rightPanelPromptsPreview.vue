<style scoped>
.card {
  padding: 1rem;
  overflow: hidden;
  border: 1px solid #c5c5c5;
  border-radius: 12px;
  background-color: #d9d9d92f;
  backdrop-filter: blur(8px);
  min-width: 344px;
}

.wrap {
  display: flex;
  flex-direction: column;
  gap: 1rem;
  position: relative;
  z-index: 10;
  border: 0.5px solid #525252;
  border-radius: 8px;
  overflow: hidden;
}

.terminal {
  display: flex;
  flex-direction: column;

  font-family:
    ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, 'Liberation Mono', 'Courier New',
    monospace;
}

.head {
  display: flex;
  align-items: center;
  justify-content: space-between;
  overflow: hidden;
  min-height: 40px;
  padding-inline: 12px;
  border-top-left-radius: 8px;
  border-top-right-radius: 8px;
  background-color: #202425;
}

.title {
  display: flex;
  align-items: center;
  gap: 8px;
  height: 2.5rem;
  user-select: none;
  font-weight: 600;
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
  color: #bcbcbc;
}

.title > svg {
  height: 18px;
  width: 18px;
  margin-top: 2px;
  color: #006adc;
}

.copy_toggle {
  display: flex;
  align-items: center;
  justify-content: center;
  padding: 0.25rem;
  border: 0.65px solid #c1c2c574;
  margin-left: auto;
  border-radius: 6px;
  background-color: #202425;
  color: #8e8e8e;
  cursor: pointer;
  transition: all 0.3s ease;
}

.copy_toggle:hover,
.clear_toggle:hover {
  color: #ffffff;
  border-color: #c1c2c5;
}

.copy_toggle > svg {
  width: 20px;
  height: 20px;
}

.clear_toggle {
  display: flex;
  align-items: center;
  justify-content: center;
  padding: 0.25rem;
  border: 0.65px solid #c1c2c574;
  border-radius: 6px;
  background-color: #202425;
  color: #8e8e8e;
  cursor: pointer;
  transition: all 0.3s ease;
}

.clear_toggle > svg {
  width: 20px;
  height: 20px;
}

.copy_toggle:active > svg > path,
.copy_toggle:focus-within > svg > path {
  animation: clipboard-check 500ms linear forwards;
}

.body {
  display: flex;
  flex-direction: column;
  position: relative;
  border-bottom-right-radius: 8px;
  border-bottom-left-radius: 8px;
  overflow-x: auto;
  overflow-y: auto;
  padding: 1rem;
  line-height: 19px;
  color: white;
  background-color: black;
  white-space: normal;
  word-wrap: break-word;
  max-height: 300px;
}

.pre {
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  white-space: normal;
  background-color: transparent;
  overflow: visible;
  box-sizing: border-box;
  font-size: 16px;
  word-break: break-all;
  width: 100%;
}

.pre code:nth-child(1) {
  color: #e34ba9;
}

.pre code:nth-child(2) {
  color: #ffffffbd;
}

.cmd {
  position: relative;
  display: inline-block;
  min-height: 19px;
}

@keyframes clipboard-check {
  100% {
    color: #fff;
    d: path(
      'M 9 5 H 7 a 2 2 0 0 0 -2 2 v 12 a 2 2 0 0 0 2 2 h 10 a 2 2 0 0 0 2 -2 V 7 a 2 2 0 0 0 -2 -2 h -2 M 9 5 a 2 2 0 0 0 2 2 h 2 a 2 2 0 0 0 2 -2 M 9 5 a 2 2 0 0 1 2 -2 h 2 a 2 2 0 0 1 2 2 m -6 9 l 2 2 l 4 -4'
    );
  }
}

/* 滚动条样式 */
.body::-webkit-scrollbar {
  width: 6px;
}

.body::-webkit-scrollbar-track {
  background: #2a2a2a;
  border-radius: 3px;
}

.body::-webkit-scrollbar-thumb {
  background: #444;
  border-radius: 3px;
}

.body::-webkit-scrollbar-thumb:hover {
  background: #555;
}
</style>

<template>
  <div v-if="promptsStore.showPreview">
    <div class="wrap">
      <div class="terminal">
        <hgroup class="head">
          <p class="title">
            <svg
              width="16px"
              height="16px"
              aria-hidden="true"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 24 24"
              stroke-linejoin="round"
              stroke-linecap="round"
              stroke-width="2"
              stroke="currentColor"
              fill="none"
            >
              <path
                d="M7 15L10 12L7 9M13 15H17M7.8 21H16.2C17.8802 21 18.7202 21 19.362 20.673C19.9265 20.3854 20.3854 19.9265 20.673 19.362C21 18.7202 21 17.8802 21 16.2V7.8C21 6.11984 21 5.27976 20.673 4.63803C20.3854 4.07354 19.9265 3.6146 19.362 3.32698C18.7202 3 17.8802 3 16.2 3H7.8C6.11984 3 5.27976 3 4.63803 3.32698C4.07354 3.6146 3.6146 4.07354 3.32698 4.63803C3 5.27976 3 6.11984 3 7.8V16.2C3 17.8802 3 18.7202 3.32698 19.362C3.6146 19.9265 4.07354 20.3854 4.63803 20.673C5.27976 21 6.11984 21 7.8 21Z"
              ></path>
            </svg>
            提示词预览
          </p>

          <div style="display: flex; gap: 8px">
            <button
              class="copy_toggle"
              tabindex="-1"
              type="button"
              @click="copyToClipboard"
              title="复制"
            >
              <svg
                width="16px"
                height="16px"
                aria-hidden="true"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
                stroke-linejoin="round"
                stroke-linecap="round"
                stroke-width="2"
                stroke="currentColor"
                fill="none"
              >
                <path
                  d="M9 5h-2a2 2 0 0 0 -2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2 -2v-12a2 2 0 0 0 -2 -2h-2"
                ></path>
                <path
                  d="M9 3m0 2a2 2 0 0 1 2 -2h2a2 2 0 0 1 2 2v0a2 2 0 0 1 -2 2h-2a2 2 0 0 1 -2 -2z"
                ></path>
              </svg>
            </button>
            <button
              class="clear_toggle"
              tabindex="-1"
              type="button"
              @click="handleClear"
              title="清空"
            >
              <svg
                width="16px"
                height="16px"
                aria-hidden="true"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
                stroke-linejoin="round"
                stroke-linecap="round"
                stroke-width="2"
                stroke="currentColor"
                fill="none"
              >
                <path
                  d="M4 7l16 0M10 11l0 6M14 11l0 6M5 7l1 12a2 2 0 0 0 2 2h8a2 2 0 0 0 2 -2l1 -12M9 7V4a1 1 0 0 1 1 -1h4a1 1 0 0 1 1 1v3"
                ></path>
              </svg>
            </button>
          </div>
        </hgroup>

        <div class="body">
          <pre class="pre">
            <code>Product By Next Promots:&nbsp;</code>
            <code>
                {{ promptsStore.previewPromptsValue || '暂无选中提示词' }}
            </code>
          </pre>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
import { aiPromptsSelectedStore } from '@/stores/aiPromptsSelected-store'
import { message } from 'ant-design-vue'

const promptsStore = aiPromptsSelectedStore()

const copyToClipboard = async () => {
  const text = promptsStore.previewPromptsValue

  if (!text || text === '暂无选中提示词') {
    message.error('没有提示词可复制')
    return
  }

  // 现代 Clipboard API
  if (navigator.clipboard && window.isSecureContext) {
    try {
      await navigator.clipboard.writeText(text)
      message.success('复制成功')
      return
    } catch (err) {
      console.warn('clipboard api 失败，使用兜底方案', err)
    }
  }

  // 兜底方案（兼容 Electron / file://）
  try {
    const textarea = document.createElement('textarea')
    textarea.value = text
    textarea.style.position = 'fixed'
    textarea.style.left = '-9999px'
    textarea.style.top = '-9999px'
    document.body.appendChild(textarea)
    textarea.focus()
    textarea.select()

    const success = document.execCommand('copy')
    document.body.removeChild(textarea)

    success ? message.success('复制成功') : message.error('复制失败')
  } catch (err) {
    console.error(err)
    message.error('复制失败')
  }
}

const handleClear = () => {
  promptsStore.clearPreview()
  message.success('已清空提示词')
}
</script>
